    ...
    <div class="row">
    	<div id="content" class="cell" >
			...
			while($ld[$i]=mysql_fetch_assoc($lq)){
				$ld[$i]["img"]=mysql_fetch_assoc(mysql_query("SELECT * FROM `ag_media` WHERE `dep_table`='look' AND `dep_id`='".$ld[$i]["id"]."' AND `mostrar`='1' LIMIT 1"));
				$ld[$i]["img"]=$ld[$i]["img"]["url"];
				$i++;
			}
			unset($ld[$i]);
			if(count($ld)==0){
            ...
            <div class="largePic"><img src="http://silenzio.com.ar/cp/uploads/box/<?=$ld[0]["img"]?>"  alt="<?=$ld[0]["descripcion"]?>"/></div>
            <ol class="smallPic">
            	<?
				foreach($ld as $d){
					echo '<li><img src="http://silenzio.com.ar/cp/uploads/box/'.$d["img"].'" alt="'.$d["descripcion"].'" title="'.$d["titulo"].'"/></li>';
				}
				?>
            </ol>
        </div>
