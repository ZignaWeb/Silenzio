...
if ($u_n>0) {
	...
	for ($i=0;$i<$u_n;$i++) {
		...
		echo '<li class="itemAdmin">
					<a href="./?do=eliminar&que='.$tabla.'&id='.$u_d[$tabla."id"].'" class="opciones" title="eliminar '.$tabla.'"><img src="imgs/eliminar.png" width="20" height="20" alt="eliminar" /></a>
					...
					<div class="thumbtxt">
						<span class="titulo">'.$u_d["titulo"].'</span><br>
						<span class="descripcion">'.$u_d["descripcion"].'</span>
						<br><span class="fecha">'.$u_d["inicio"].' a '.$u_d["fin"].'</span>
					</div>
			</li>';
	}
	echo '</ol>';
