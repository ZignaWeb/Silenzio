		<div id="contenido">
                	...
                	<div id="megaWindContenido">
                    	<div id="bigList" class="camp">
                            <ol id="BIGThumbs">
                                ...
                                $tabla="campania";
                                $u_q=mysql_query("SELECT * FROM `$tabla` WHERE 1 ORDER BY `campaniaid` ASC");
                                $u_n=mysql_num_rows($u_q);
                                for ($i=0;$i<$u_n;$i++) {
                                    $u_d[$i]=mysql_fetch_assoc($u_q);
                                    echo '<li><img class="thumb" src="../nueva/include/img/content/box/'.$u_d[$i]["img"].'" alt="'.$u_d[$i]["titulo"].']['.$u_d[$i]["descripcion"].'" /></li>';
                                }
                                ?>
                            </ol>
            ...
            <script type="text/javascript">
				...
				$("img.thumb").click(function(){
					...
					change = $(this).attr("src").replace("/box/","/big/");
